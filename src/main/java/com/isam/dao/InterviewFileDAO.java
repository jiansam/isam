...
public class InterviewFileDAO {
	public static Hashtable<String, String> upload(HttpServletRequest request){
		...
		if(ServletFileUpload.isMultipartContent(request)){
			...
			try{
				...
				stmt = sqltool.prepare("INSERT INTO InterviewFile (Filename, Content, interviewIdentifier, Purpose, UploadDate"
						  + ") VALUES (?, ?, ?, ?, GETDATE())");
				...
				stmt = sqltool.prepare("UPDATE InterviewFile SET FileSize = DATALENGTH(Content) WHERE interviewIdentifier = ? ");
	...
	public static void delete(int identifier){
		...
		try{
			sqltool = new SQL();
			stmt = sqltool.prepare("DELETE FROM InterviewFile WHERE identifier = ?");
