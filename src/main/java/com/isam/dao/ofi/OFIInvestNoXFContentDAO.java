...
public class OFIInvestNoXFContentDAO {
	public void delete(String serno){
		SQL sqltool = new SQL();
		String forStmt = "DELETE FROM OFI_InvestNoXFContent WHERE serno = ?";
		try {
	...
	public void insert(List<OFIInvestNoXFContent> beans) {
		String forpstmt = "insert into OFI_InvestNoXFContent values(?,?,?)";
		SQL sqltool = new SQL();
		try {
			sqltool.noCommit();
	...
	public Map<String,String> selectBySerno(String serno){
		SQL sqltool = new SQL();
		String forStmt = "SELECT optionId,value FROM OFI_InvestNoXFContent where serno=?";
