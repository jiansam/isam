...
public class SurveyFileDAO
{
	...
	public Map<Integer, Map<String, SurveyFile>> getAll()
	{
		...
		try {
			pstmt = sqltool.prepare("SELECT id, year, type, file_title FROM SurveyFile WHERE disable=0 order by year DESC, type ASC");
	...
	public void insertSurveyFile(SurveyFile bean){
		...
		PreparedStatement pstmt = null;
		try {
			pstmt = sqltool.prepare("INSERT INTO SurveyFile(year,type,file_title,file_content,disable,updateTime) VALUES(?,?,?,?,?,GETDATE())");
			int column = 1;
	...
	public void updateSurveyFile(SurveyFile bean){
		...
		PreparedStatement pstmt = null;
		try {
			pstmt = sqltool.prepare("UPDATE SurveyFile set "
								  + "file_title = ?,"
								  + "file_content = ?,"
								  + "updateTime = GETDATE()"
								  + "WHERE id = ? ");
	...
	public boolean isExist(int year, String type)
	{
		...
		try {
			pstmt = sqltool.prepare("SELECT year, type FROM SurveyFile WHERE year=? AND type=? AND disable=0");
			int column = 1;
