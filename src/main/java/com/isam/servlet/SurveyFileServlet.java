...
import com.isam.dao.SurveyFileDAO;

import Lara.Utility.DateUtil;
import Lara.Utility.ToolsUtil;

public class SurveyFileServlet extends HttpServlet {
	...
	private void doIndex(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		if("front".equals(doThing)) {
			//前台轉頁
			response.sendRedirect(path + "/survey/surveytemplate.jsp");
		}else {
			//後台轉頁
			response.sendRedirect(path + "/console/survey/surveytemplate.jsp");
		}
		return;
	}
}
