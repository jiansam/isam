...
<script type="text/javascript">
	$(function(){
		 ...
		 var y=$.extend({
				"aaSorting": [[ 1, "asc" ]],"aoColumnDefs": [{ 'sClass':'center', "aTargets": [0,1,5] },
			...
			$("body").on('click',"#mScore tbody tr td:has(.mEdit)",function(){
				var ntr= $(this).parent();
				var nTds=$('td',ntr);
				$.post( "${pageContext.request.contextPath}/console/cnfdi/content/newmscore.jsp",
						{'year':$('td:eq(1)',ntr).text(),'investNo':"${sysinfo.INVESTMENT_NO}"
					,'score':$('td:eq(2)',ntr).text(),'oscore':$('td:eq(3)',ntr).text()
					,'nscore':$('td:eq(4)',ntr).text()}, function(data){
					$( "#mForm" ).html(data); 
					$( "#mForm" ).dialog({
					      height:300,
					      width:600,
...
<c:if test="${not empty manage}">
	 ...
	 <table id='mScore' style="width: 95%;font-size: 16px;margin-left: 15px;">
		 <thead>
			<tr>
				<th>序號</th>
				<th>年度</th>
		...
		<tbody>
			<c:forEach items="${manage}" var="mitem">
			 	<tr>
			 		...
			 		<td><c:if test="${fn:contains(memberUrls,'E0401')}"><input style="font-size: 14px;" class="mEdit btn_class_opener" type="button" value="編輯"></c:if></td>
			 	</tr>
	 		</c:forEach>
		</tbody>
	 </table>
