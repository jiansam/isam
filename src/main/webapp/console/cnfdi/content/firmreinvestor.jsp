...
<c:if test="${not empty reInvests}">
	...
	<c:forEach var="re" items="${reInvests}">
		...
		<div>
			...
			<c:if test="${fn:contains(memberUrls,'E0401') ||re.updateuser =='admin'}">
			<a href="<c:url value='/console/showreinvest.jsp?investNo=${sysinfo.INVESTMENT_NO}&reinvest=${re.reInvestNo}'/>" class="btn_class_opener">編輯轉投資事業</a></c:if></div>
				...
				<tr>
					...
					<td>
						<c:choose>
							...
							<c:when test="${empty re.paidCapital}">尚無資料
								<%-- <c:choose>
