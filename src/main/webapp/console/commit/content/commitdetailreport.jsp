...
<%-- 	<c:if test="${not empty summary}"> --%>
		<div id="accordion">
			...
			<div>
				...
				<c:if test="${not empty subcrSer}">
					<div style="height: 50px;"></div>
					<strong style="margin:5px 10px;color:#CC0000;font-size: 16px;">依投資案號執行明細</strong>
					<table class="datailByInvest" style="font-size: 16px;text-align: center;width: 100%;">
						<thead><tr>
							<th rowspan="2">年度</th>
							<th rowspan="2">投資案號</th>
							<th colspan="3">國內相對投資計畫</th>
							<th rowspan="2">資金回饋</th>
						</tr>
						<tr>
							<th class="sortInput">上半年</th>
							<th class="sortInput">下半年</th>								
						...
						<c:forEach var="povL" items="${subcrSer}">
							<tr>
								<c:forEach var="repserno" items="${povL}" varStatus="i">
								<td>
									<c:if test="${i.index <2}">${repserno}</c:if>
									<c:if test="${i.index >1 && repserno!='' }">
										<input type="button" onclick="toEditDateilPage('${repserno}')" class="myEdit btn_class_opener" style="text-align: center;font-size: 12px;" value="${param.EditType}"/>
									</c:if>
								</td>
								</c:forEach>
							</tr>
			...
			<c:forEach var="sumItem" items="${summary}">
				...
				<div style="min-height: 300px;">
						...
						<tr>
							<c:forEach var="item" items="${item}" varStatus="i">
								<c:choose>
									...
									<c:when test="${i.index eq 1}"><td style="text-align: center;">${repTypeMap[item]}</td></c:when>
									...
									<c:otherwise><td style="text-align: center;"><span>${item}</span></td></c:otherwise>
								</c:choose>
							</c:forEach>
						</tr>
					</c:forEach>
					...
					<c:if test="${not empty subcpd && sumItem.key eq '01'}">
						<br/><br/>
						<strong style="margin:5px 10px;color:#CC0000;font-size: 16px;">案號明細加總</strong>
							<table class="formProj">
								<tr>
									<th rowspan="2" style="width: 20%;">填報項目</th>
									<th colspan="2">管制期間</th>
									<th rowspan="2">原始明細承諾金額</th>
									<th rowspan="2">累計明細執行承諾金額</th>
									<th rowspan="2" style="width: 12%;">累計達成率</th>
								</tr>
								<tr>
									<th>起始年</th>
									<th>截止年</th>
								</tr>
								<c:forEach var="itemdetail" items="${subcpd}"  varStatus="s">
									<c:choose>
										<c:when  test="${itemdetail[s.index] eq '01'}"></c:when>
										<c:otherwise>
											<tr>
												<c:forEach var="item" items="${itemdetail}" varStatus="i">
													<c:choose>
														<c:when test="${i.index eq 0}"><td>${CRType[item]}</td></c:when>
														<c:when test="${i.index eq 1 || i.index eq 2}"><td align="center">${item}</td></c:when>
														<c:when test="${i.index eq 5}">
															<td class="trRight">
															<span class="numberFmt">${item}</span>
															<c:if test="${not empty item}">%</c:if></td>
														</c:when>
														<c:otherwise><td class="trRight"><span class="numberFmt">${item}</span></td></c:otherwise>
													</c:choose>
												</c:forEach>
											</tr>
										</c:otherwise>
					...
					<c:if test="${not empty sub03cpd && sumItem.key eq '03'}">
						<br/><br/>
						<strong style="margin:5px 10px;color:#CC0000;font-size: 16px;">案號明細加總</strong>
							<table class="formProj">
								<tr>
								...
								<c:forEach var="itemdetail" items="${sub03cpd}">
									<tr>
										<c:forEach var="item" items="${itemdetail}" varStatus="i">
											<c:choose>
												<c:when test="${i.index eq 0}"><td>${CRType[item]}</td></c:when>
