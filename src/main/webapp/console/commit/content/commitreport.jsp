...
$(function(){
	...
	setYearRPRange("${bean.startYear}","${bean.endYear}");
...
function deleteCommitRP(repserno){
	$("<div style='font-size='12px;''>您即將刪除本筆資料，請確認是否繼續?</div>").dialog({
		...
		buttons: {
	        "確定": function() {
			  postUrlByForm('/console/commit/commitrpdelete.jsp',{'repserno':repserno});
	          $( this ).dialog( "close" );
	        },
	        "取消": function() {
	          $( this ).dialog( "close" );
...
<fieldset style="-moz-border-radius:6px; -moz-border-top-colors:#fff #ccc #ccc #ccc #ccc #ccc #ccc #ccc #ccc; -moz-border-right-colors:#fff #ccc #ccc #ccc #ccc #ccc #ccc #ccc #ccc; -moz-border-left-colors:#fff #ccc #ccc #ccc #ccc #ccc #ccc #ccc #ccc; -moz-border-bottom-colors:#fff #ccc #ccc #ccc #ccc #ccc #ccc #ccc #ccc;">
	<legend style="text-align:center;padding:5px;width:200px;white-space:nowrap;border-top:1px solid #E6E6E6;margin-bottom: 10px;background-color: #66cccc;-webkit-border-radius: 30px; -moz-border-radius: 30px;border-radius: 30px;">
		<c:if test="${param.editType eq 'add'}"><c:set var="typeEdit"  value="新增" /></c:if>
		<c:if test="${param.editType eq 'edit'}"><c:set var="typeEdit"  value="修改" /></c:if>
		<span style="color:#F30;"><strong>&nbsp;${typeEdit}執行情形&nbsp;</strong>&nbsp;</span>
	</legend>
	...
	<form  id="CRForm" action="<c:url value='/console/commit/addcommitreport.jsp'/>" method="post">
		<div id="tabs" class="tabs-bottom" style="font-size: 16px;margin-top: 20px;">
			<ul>
		    	...
		    	<li style="float: right"><input type="button" class="btn_class_opener" onclick="postUrlByForm('/console/commit/showcommitdetail.jsp',{'serno':'${param.idno}'})" value="回企業編輯" /></li>
			    <c:if test="${fn:contains(memberUrls,'E0102')}">
			    	<li style="float: right"><input type="button" onclick="goToEditPage('${serno}','${param.idno}','${param.investor}')" class="myEdit btn_class_opener" value="編輯原承諾"/></li>
				</c:if>
			    <c:if test="${param.editType eq 'edit'}">
			    	<li style="float: right"><input type="button" class="btn_class_opener" onclick="deleteCommitRP('${param.repserno}')" value="刪除紀錄" /></li>
			    </c:if>
			  </ul>
			 ...
			 <div id="tabs-1">
				<fieldset style="-moz-border-radius:6px; -moz-border-top-colors:#fff #ccc #ccc #ccc #ccc #ccc #ccc #ccc #ccc; -moz-border-right-colors:#fff #ccc #ccc #ccc #ccc #ccc #ccc #ccc #ccc; -moz-border-left-colors:#fff #ccc #ccc #ccc #ccc #ccc #ccc #ccc #ccc; -moz-border-bottom-colors:#fff #ccc #ccc #ccc #ccc #ccc #ccc #ccc #ccc;">
					<legend style="border-top:1px solid #E6E6E6;margin-bottom: 0px;">
						...
						投資人名稱：
						<c:choose>
							<c:when test="${fn:length(param.investor)>14}"><a title="${param.investor}">${fn:substring(param.investor,0,13)}...</a></c:when>
							<c:otherwise>${param.investor}</c:otherwise>
						</c:choose>
						</span>
						<span class="basetable">
						...
						<c:if test="${bean.repType eq '01'}">
							...
							列管期數：<span id="nowCountRN">${conutRN-1}</span>/${conutRN}
						</c:if>
						<c:if test="${bean.repType eq '0103'}">
							<c:set var="conutRN" value="${(bean.endYear-bean.startYear)+1}" />
							列管期數：<span id="nowCountRN">${conutRN}</span>/${conutRN}
						</c:if>
						</span>
						</c:if>
						<span class="basetable">
						...
						<span class="basetable">
							填報年度：<c:if test="${param.editType eq 'edit'}">${crBean.year}<c:if test="${not empty crBean.year}">年</c:if></c:if>
							<select style="width: 80px;" name="year">
							</select>
							<input type="hidden" name="restrainType" value="${bean.type}"/>
							<input type="hidden" name="IDNO" value="${param.idno}"/>
							<input type="hidden" name="editType" value="${param.editType}"/>
							<input type="hidden" name="sernoStr" value="${serno}"/>
							<input type="hidden" name="investor" value="${param.investor}"/>
							<input type="hidden" name="repSerno" value="${param.repserno}"/>
						</span>
						<c:if  test="${bean.type eq '01'}">
						<br/>
							<span class="basetable">
						...
						<br/><br/>
						<c:if test="${empty detailBean && bean.type eq '01'}"><span class="basetable">必須先有管制項目，方可建置執行情形</span></c:if>
						<c:if test="${not empty detailBean}"><strong style="margin:10px 10px;color:#222;">${typeName}-填報年執行情形<c:if test="${bean.type eq '01'}">（新台幣元）</c:if></strong></c:if>
							...
							<c:choose>
								...
								<c:when test="${bean.type eq '02'}">
									<table class="formProj">
										...
										<tr style="text-align: center;">
											<td><input type="text" name="RsType0201"  class="nextFocus numberFmt trRight"/>%</td>
											<td><input type="text" name="RsType0202"  class="nextFocus numberFmt trRight"/>%</td>
											<td><input type="text" name="RsType0203"  class="nextFocus numberFmt trRight"/>%</td>
										</tr>
									</table>
								</c:when>
								<c:when test="${bean.type eq '03'}">
									...
									<table class="formProj" style="text-align: right;">
										<tr style="text-align: center;">
											...
											<c:if test="${not empty detailEx}">
												<th style="width: 15%;">原承諾金額(<span id="changeRepYear">${bean.endYear}</span>)</th>
											</c:if>
