...
$(function() {
	...
	// 			ajaxByDataToDialog($(this))
			...
			postUrlByForm('/showinvestor.jsp',{'banno':banno});
		}
	});
});
</script>
		...
		<table id="example" class="display" style="width: 98%;"> 
			...
			<tbody>
				<c:forEach var="item" items="${ofiInvestors}">
					<tr>
					<td></td>
					<td 	<c:if test="${item.investNoStrs==''}">data-banno="${item.note}"</c:if> data-investorseq="${item.investorSeq}">${item.companytype}</td>
					<td>${item.investNoStrs}</td>
