...
/*	處理 Datatable 不送隱藏物件的問題	*/
$(document).ready(function() {
	$('form').submit( function() {
		var checkedbox = $('input:checked', $("#example").dataTable().fnGetNodes());
		if(checkedbox.length == 0){
			alert("請選擇至少一個訪查企業");
			return false;
		}

		var newform = $(this).clone().hide();
		$('body').append(newform);
		
		$(newform).find('input:checked').remove();
		for(var i = 0;i < checkedbox.length;i++){
			$(newform).append($(checkedbox[i]).clone());
		}
