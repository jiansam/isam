...
$.extend($.ui, { ympicker: { version: "1.10.3" } });
...
$.ympicker.version = "1.10.3";

})(jQuery);
jQuery(function($){
	$.ympicker.regional['zh-TW'] = {
		closeText: '關閉',
		prevText: '&#x3C;上年',
		nextText: '下年&#x3E;',
		currentText: '今天',
	...
	$.ympicker.setDefaults($.ympicker.regional['zh-TW']);
});
$(function() {
	 var old_generateMonthYearHeader = $.ympicker._generateMonthYearHeader;
    var old_get = $.ympicker._get;
    var old_CloseFn = $.ympicker._updateDatepicker;
    $.extend($.ympicker, {
        _generateMonthYearHeader:function (a,b,c,d,e,f,g,h) {
            var htmlYearMonth = old_generateMonthYearHeader.apply(this, [a, b, c, d, e, f, g, h]);
            if ($(htmlYearMonth).find(".ui-datepicker-year").length > 0) {
                htmlYearMonth = $(htmlYearMonth).find(".ui-datepicker-year").find("option").each(function (i, e) {
               	 if(parseInt(e.value,10) - 1911 > 0){
               		 $(e).text(e.value).text(parseInt(e.value,10) - 1911);
               	}
...
//               	 if(Number(e.value) - 1911 > 0){$(e).text(Number(e.innerText) - 1911);}
       ...
       _setDateDatepicker: function (a, b) {
        ...
        _selectDate: function(id, dateStr) {
    		...
    		dateStr = (dateStr != null ? dateStr : this._formatDate(inst));
    		if (inst.input) {
    			var dateFormate = inst.settings.dateFormat == null ? "yy/mm" : inst.settings.dateFormat; 
    			var vAry = dateStr.split("/");
...
/*設定起訖年月*/
function setYMFromTo($Max,$Min,MaxVal,MinVal){
	var MaxY=parseInt(MaxVal.substring(0,3),10)+1911;
	var MaxM=parseInt(MaxVal.substring(3,5),10)-1;
	var MinY=parseInt(MinVal.substring(0,3),10)+1911;
	var MinM=parseInt(MinVal.substring(3,5),10)-1;
	$Max.ympicker({
		minDate: new Date(MinY, MinM, 1),
		maxDate: new Date(MaxY, MaxM, 1),
		onClose: function( selectedDate ) {
			if(selectedDate<$Min.val()){
				alert("選擇日期不可小於起始年月，請重新選擇");
				$(this).val($Min.val()).focus();
			}	        
	    }
	});
	$Min.ympicker({
		...
		maxDate: new Date(MaxY, MaxM, 1),
		onClose: function( selectedDate ) {
			if(selectedDate>$Max.val()){
				alert("選擇日期不可大於迄日，請重新選擇");
				$(this).val($Max.val()).focus();
			}
	     }
	});
}
